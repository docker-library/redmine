...
_fix_permissions() {
	...
	if [ "$(id -u)" = '0' ]; then
		# 2019/04/23 Chai 
		# Skip files because it is too slow to modify permissions under files when start a container with over 50,000 files/directories mounted
		# find config files log public/plugin_assets \! -user redmine -exec chown redmine:redmine '{}' +
		find config log public/plugin_assets \! -user redmine -exec chown redmine:redmine '{}' +
	fi
	...
	find config log public/plugin_assets tmp -type d \! -perm 755 -exec chmod 755 '{}' + 2>/dev/null || :
	find config log public/plugin_assets tmp -type f \! -perm 644 -exec chmod 644 '{}' + 2>/dev/null || :
}
